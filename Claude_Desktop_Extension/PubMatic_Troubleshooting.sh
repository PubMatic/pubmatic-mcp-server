...
# Runs 7 ordered checks: curl, network, DNS, platform, Python, SSL, MCP health.
...
###############################################################################
check_curl() {
    echo "[1/7] Checking for curl..."
    log "[1/7] curl availability check"
...
build_ssl_fix_steps() {
    ...
    if [ "$DETECTED_OS" = "Darwin" ]; then
        ...
        if [ -f "$cert_cmd" ]; then
            SSL_STEPS+=("sudo bash \"${cert_cmd}\"|Run Apple's certificate installer for Python ${PYTHON_MINOR}")
        fi
    fi
}
...
print_summary() {
    ...
    if [ "$CHECK_CURL" = "fail" ] || [ "$CHECK_NETWORK" = "fail" ] || [ "$CHECK_DNS" = "fail" ] || \
